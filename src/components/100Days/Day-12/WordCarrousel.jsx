...
const carrouselWords = ["ungovernable", "stronger", "a force of nature"];

export default function WordCarrousel() {
  const [currentWord, setCurrentWord] = useState(carrouselWords[0]);

  useEffect(() => {
    const carrouselChanger = setTimeout(() => {
      setCurrentWord((prevWord) => {
        const INDEX =
          carrouselWords.indexOf(prevWord) + 1 === carrouselWords.length
            ? 0
  ...
  return (
    <Frame>
      <SectionTitle> World Carrousel </SectionTitle>
