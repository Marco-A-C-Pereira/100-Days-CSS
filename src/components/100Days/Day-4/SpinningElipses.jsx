...
function Elipse({ rotation, tailwindSize, color, delay }) {
  return (
    <motion.div
      ...
      className={`border-2 ${color} h-${tailwindSize} w-${tailwindSize} absolute top-1/2 left-1/2 `}
    ></motion.div>
  );
}
...
export default function SpinningElipses() {
  return (
    <Frame>
      <SectionTitle>
        <p className="leading-snug font-sans font-bold">(23/4/23)</p>
        Animação Elipses{" "}
      </SectionTitle>
      ...
      <SrcButton src={"https://100dayscss.com/days/65/"}> </SrcButton>
    </Frame>
  );
}
