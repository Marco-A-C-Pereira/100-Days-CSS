...
function SparkleContainer({ isChecked }) {
  ...
  const sparkles = Array.from({ length: TOTALSPARKLES }, (_, index) => {
      ...
      >
        <motion.div animate={{ rotate: angle }}>
          <motion.div
            ...
            className={` w-8 h-[1px] bg-sky-300 -z-10 `}
          ></motion.div>
        </motion.div>
      </motion.div>
    );
  ...
  return (
    <label className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 flex z-10">
      <input
        ...
        type="checkbox"
        onClick={() => handleClick((prevIsChecked) => !prevIsChecked)}
      />
      <div
      ...
      >
        <div
          className={`${INNERINGSTYLE} bg-stone-300 rounded-full trasition duration-500`}
        ></div>
      </div>
    </label>
  );
...
export default function SparkleCheckbox() {
  ...
  return (
    <Frame>
      <SectionTitle>Sparkle Checkbox</SectionTitle>
      <article className="relative bg-slate-900 h-52 w-52">
        <RoundedCheckbox isChecked={isChecked} handleClick={setisChecked} />
